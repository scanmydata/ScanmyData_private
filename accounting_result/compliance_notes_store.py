...
Per-company, per-year resolutions for the two monthly-completeness checks
this module backs:
...
year), so the same question isn't asked again on every future computation
of that same year.
...
def set_payroll_check(
    ...
    """`resolution`: "skip" (proceed as-is; remembered so the warning stops
    firing for this company/year specifically — e.g. it genuinely stopped
    having payroll that year) or "manual" (the accountant keyed in totals
    for the missing months, stored in `monthly_totals` as {"YYYY-MM": amount}
    and ADDED on top of whatever myDATA itself already reported for code 581
    on every future computation of this year — see
    accounting_result.engine.build_report's payroll_manual_addition)."""
    data = _read(path)

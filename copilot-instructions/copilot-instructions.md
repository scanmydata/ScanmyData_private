...
Βήματα σύνδεσης (Login)
1. Μεταβείτε στην σελίδα σύνδεσης: `https://YOUR_APP_DOMAIN/login`
2. Περίμενε έως ότου εμφανιστεί το πεδίο χρήστη, π.χ. selector `#username` ή `input[name="email"]`.
3. Συμπληρώστε το όνομα χρήστη (από secrets).
4. Περίμενε έως ότου εμφανιστεί το πεδίο κωδικού, π.χ. `#password`.
5. Συμπληρώστε τον κωδικό (από secrets).
6. Κάντε κλικ στο κουμπί σύνδεσης, π.χ. `button[type="submit"]`.
7. Περίμενε για ένα στοιχείο που υποδεικνύει επιτυχή σύνδεση (π.χ. `nav .user-avatar`, ή redirect στο `/dashboard`).
8. Επιβεβαίωση: αν δεν εντοπιστεί το στοιχείο μετά από timeout (π.χ. 15s), πάρτε screenshot, log το σφάλμα και επαναλάβετε μέχρι 2 φορές πριν αποτύχετε.

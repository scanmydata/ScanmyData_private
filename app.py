...
def api_accounting_result_vat_profile_detect():
    ...
    try:
        ...
        # "Υπαγωγή ΦΠΑ: ΝΑΙ" while its actual "Καθεστώς ΦΠΑ" is an exemption
        # regime — confirmed on two real companies, one tagged
        # "ΑΠΑΛΛΑΣΣΟΜΕΝΩΝ" (VAT-exempt) and one "ΕΙΔΙΚΟ ΕΓΧΩΡΙΟ ΚΑΘΕΣΤΩΣ
        # ΜΙΚΡΩΝ ΕΠΙΧΕΙΡΗΣΕΩΝ" (small-business exemption scheme) — both of
        # which mean the company is NOT actually subject to VAT for this
        # report's purposes, regardless of the ΝΑΙ flag. The exact XML tag
        # name for "Καθεστώς ΦΠΑ" isn't confirmed, so scan every tag VALUE
        # for the regime text instead of relying on one specific key.
        regime_text = " ".join(str(v) for v in all_tags.values()).upper()
